...
function shopifyDescRuleOther(descImgN: number, isKeycapSet: boolean): string {
  ...
  return [
    ...
    "PROFİL SCULPT: Cherry Profile = row-specific sculpt (her sıra farklı). Uniform profil (DSA/XDA/KAM/FOA/MOA/MAO) = tüm sıralarda aynı kap şekli. İki profil için de 'row-by-row sculpted' YAZMA — ayır.",
...
function shopifyDescRuleStacked(isKeycapSet: boolean): string {
  ...
  isKeycapSet
    ...
    : "",
  isKeycapSet
    ? "PROFİL SCULPT: Cherry Profile = row-specific sculpt (her sıra farklı yükseklik/açı). Uniform bir profil (DSA/XDA/KAM/FOA/MOA/MAO — operatörün belirttiği) = tüm sıralarda AYNI kap şekli, sıradan sıraya değişmez. Spec ve karşılaştırma tablosunda İKİ profil için de 'row-by-row sculpted' YAZMA — Cherry'yi 'row-specific', uniform profili 'same shape on every row' diye ayır. Operatörün verdiği profil adına göre yaz."
    : "",
  ...
  isKeycapSet
    ...
    : "",
  "İLETİŞİM NOTU: FAQ'nun ilk cevabına VE/VEYA `bm-note`'a kısa bir satır ekle — uyumluluk sorusu olan müşteri mağazayla iletişime geçebilir / sipariş öncesi yazabilir.",
...
function manusDescRuleCompact(isStacked: boolean, isKeycapSet: boolean): string {
  if (isStacked) {
    return [
      ...
      "KATI: `.bm-reveal` başta görünür olmalı (opacity:1) — JS'e bağımlı gizleme yok. 'Enter key'/'Enter'/tek başına Enter YASAK, 'ANSI & ISO layout compatible' kullan. Ülke bayrağı emoji YASAK. Cherry Profile = row-specific sculpt; diğer (DSA/XDA/KAM/FOA/MOA/MAO) = uniform/aynı şekil, ikisini karıştırma. `.bm{}` DIŞINDA sabit hex/rgb yazma, hepsi `var(--…)`; ürün rengine uygun palet seç (turuncu değilse turuncu kullanma). Spec tablosunda Brand/Marka satırı YOK; keycap/aksesuarsa 'Keyboard included' satırı YOK.",
