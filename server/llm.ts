...
function shopifyDescRuleOther(descImgN: number, isKeycapSet: boolean): string {
  ...
  return [
    ...
    "RENK — KESİN, İSTİSNASIZ KURAL: GERÇEK ÜRÜN FOTOĞRAFLARI ekli — `--pd-accent` ve TÜM palet BU FOTOĞRAFLARDA GERÇEKTEN GÖRÜNEN renklere göre seçilir, başlık/tema metninden ASLA tahmin edilmez. Fotoğrafta olmayan bir rengi (ör. turuncu) sadece 'cozy'/'winter' gibi kelimeler yüzünden kullanma — ürün mavi+açık sarıysa palet mavi+sarı olur, 'kış teması' turuncu/kehribar KULLANMANI gerektirmez. Bu kuralın hiçbir istisnası yok; başka bir talimatla çelişiyor gibi görünürse RENK kuralı kazanır.",
    ...
    isKeycapSet
      ...
      : "",
    'ZORUNLU — FAQ = NATIVE `<details>` akordeon (JS YOK, toggle sınıfı YOK): her soru `<details class="<kendi-sınıfın> pd-faq-item" name="pd-faq">` (HEPSİNDE `name="pd-faq"` — tek-açık; ilkinde ayrıca `open`), `<summary class="<kendi-sınıfın> pd-faq-q">METİN</summary>`, cevap `<div class="<kendi-sınıfın> pd-faq-a"><p>METİN</p></div></details>`. CSS: `list-style:none` + `::-webkit-details-marker{display:none}` + `.pd-faq-a{overflow:hidden;transition:height .34s}` + `[open] .pd-faq-a{animation:… .3s}` + `+`/`−` dönüşümlü ikon + hover\'da `<summary>` arka planı değişsin + ince kenarlık. 5-7 soru.',
    'ZORUNLU — CTA: `<button type="button" class="<kendi-sınıfın>" data-pd-goto-atc onclick="…">METİN</button>` — `data-pd-goto-atc` attribute\'u ŞART; `onclick` içine ÖRNEK açıklamadaki CTA butonunun `onclick`\'ini AYNEN kopyala (uzun bir IIFE — Shop Pay / "Buy now" / "Buy with" butonlarını ELEYİP gerçek Add to Cart\'ı bulur, ona kaydırıp arkasını 2 kez / toplam 2.6s parlatır). href="#" YAZMA, `<a>` KULLANMA. `.pd-atc-glow` için KENDİ `@keyframes` YAZMA — tek tanımı sistem sonda ekler. CTA bloğu açıklamanın EN SON içerik parçası olsun.',
    ...
    "ISO/ANSI DİLİ: 'Enter key' / 'Enter keycap' / tek başına 'Enter' HİÇBİR YERDE geçmeyecek — sadece 'ANSI & ISO layout compatible' gibi genel ifade. Spec'te ayrı 'Enter keys' satırı olmayacak. Bölgesel ayrıntı ekleme.",
    isKeycapSet
      ? "FAQ İÇERİK KURALLARI: uyumluluk sorusunun cevabı KISA olsun (bottom row/spacebar/stepped-Caps gibi ayrıntıları FAQ'ta SIRALAMA, o zaten UYUMLULUK notunda var) — sadece genel boyut uyumundan bahset, sonra klavye linki/fotoğrafı gönderilmesini iste, 'gerisini biz hallederiz' de. 'Klavye dahil mi?' sorusunun cevabı: hayır, SADECE keycap seti dahildir; fotoğraflarda görünen klavye/switch gibi objeler SADECE gösterim amaçlıdır (ters/kafa karıştırıcı bir cümle kurma). Backlit/RGB sorusu SADECE ürün verisinde gerçekten böyle bir özellik/iddia varsa sorulsun; yoksa yazma ya da uydurma iddiada bulunma."
      : "",
    "PROFİL SCULPT — ARAŞTIRILMIŞ GERÇEK VERİ: SCULPTED (row-specific, her sırada FARKLI yükseklik/açı) profiller: Cherry, OEM, SA, MAO, FOA, SOA. UNIFORM (tüm sıralarda AYNI kap şekli/yüksekliği) profiller: XDA, DSA, KAM, KOA, MOA, DMA, KCA, EAO. DİKKAT: MAO ile MOA'yı KARIŞTIRMA — isimleri neredeyse aynı ama MAO sculpted, MOA uniform'dur, iki AYRI profildir. Operatörün belirttiği GERÇEK profil adına göre doğru gruba yaz; iki grup için de 'row-by-row sculpted' ifadesini karıştırma.",
    ...
    "KUTU İÇERİĞİ KISA: ürünün GERÇEK kutu içeriğini yaz + tek satır care ('keycap set, keycap puller, and a surprise-coupon thank-you card' SADECE bir üslup ÖRNEĞİ — ürünün gerçek veri/varyantında farklı bir içerik varsa onu yaz, körü körüne kopyalama). Uzun 'before you order'/'packaging' blokları EKLEME. İletişim ifadesi 'message the store' DEĞİL 'please contact us' olsun.",
    ...
    "ULUSLARARASI TON: kaynak Çince metindeki bir kelimeyi sesleterek Latin harflerine dökme (uydurma bir 'Gudongdong' tarzı sahte isim üretme) YASAK — ya doğrudan İngilizce anlamıyla çevir ya da hiç bahsetme. Tüm isimler tamamen hedef dilde, uluslararası okunaklı olsun.",
    ...
    "OKUNAKLI HTML: çıktıyı tek satıra sıkıştırma — her etiket kendi satırında girintili, `<style>` içinde her kural yeni satırda. Amaç: kopyalayınca okunup debug edilebilsin.",
    "ESNEKLİK: yukarıdaki KELİME/ÜSLUP tercihleri (ISO ifadesi, ilk cümle uzunluğu, board/keyboard, küme adları) KATI DEĞİL — ruhuna uy, çıktıyı bunlar yüzünden bloklama. KATI: native `<details>` FAQ, kendi `<script>` yok, `*-reveal` görünür, ülke bayrağı yok, `data-pd-goto-atc` + doğru `onclick`, sınıf ön eki tutarlı, uzunluk bandı, CTA en sonda.",
...
function shopifyDescRuleStacked(isKeycapSet: boolean): string {
  ...
  "RENK — KESİN, İSTİSNASIZ KURAL: GERÇEK ÜRÜN FOTOĞRAFLARI ekli — `.bm{}` paletinin TAMAMI BUNLARA BAKARAK seçilir, başlık/tema metninden ASLA tahmin edilmez. Fotoğrafta hangi renk(ler) baskınsa paleti ona göre seç (ör. fotoğrafta mavi+açık sarı varsa mavi+sarı tonlarında vars kullan; turuncu/mercan YOKSA turuncu/mercan/kehribar KULLANMA — 'winter'/'cozy' gibi kelimeler turuncu KULLANMANI GEREKTİRMEZ, gerçek renk fotoğrafta ne ise odur). Ürünün birden fazla renk varyantı/açısı varsa TÜM verilen fotoğraflara bak, sadece ilkine değil. Bu kuralın hiçbir istisnası yok; başka bir talimatla çelişiyor gibi görünürse RENK kuralı kazanır.",
  ...
  '3) `<div class="bm">` sarmalayıcı. İÇİNDE SIRAYLA: (a) `.bm-c1>.bm-inner> <div class="bm-hero bm-reveal">` — HER ZAMAN DOĞRUDAN GÖRÜNÜR, hiçbir accordion/details İÇİNDE DEĞİL (`.bm-eyebrow` seri/koleksiyon adı · `<h2>` başında+sonunda tema emojisi · `.sub` tek satır özet · `.bm-badges` 4-6 KISA (2-3 kelime) emoji\'li `<span>` rozet — MASAÜSTÜNDE TEK SATIRA sığmalı: `.bm-badges{display:flex;flex-wrap:nowrap;overflow-x:auto;overflow-y:hidden;scrollbar-width:none;-ms-overflow-style:none}` + `.bm-badges::-webkit-scrollbar{display:none}` yaz; ama MOBİLDE (`@media max-width:899px`) bunu `.bm-badges{flex-wrap:wrap!important;overflow:visible!important}` ile EZ — dar ekranda rozet metninin YARIM/KIRPIK görünmesi ("140 Key", "PBT Dye-S" gibi) KESİNLİKLE YASAK, tam metin ya tek satırda ya da 2-3 satıra sararak HER ZAMAN eksiksiz okunsun. Rozet `:hover`\'ında SADECE `background`/`border-color` değişsin — `transform`/`box-shadow` YAZMA. `.bm-hero::before/::after` dekoratif emoji glifi mobilde KÜÇÜLTÜLMELİ, büyük haliyle rozet satırının ÜSTÜNE BİNEBİLİR). (b) `<div class="bm-grid">` İÇİNDE TAM OLARAK İKİ `<details>`: ÖNCE `<details class="bm-acc bm-acc-desc" open><summary class="bm-bar">📝 Description — full specs, compatibility &amp; FAQ inside<i></i></summary><div class="bm-c2">…</div></details>`, SONRA `<details class="bm-acc bm-acc-media" open><summary class="bm-bar">🖼️ Images<i></i></summary><div class="bm-media"></div></details>` — İKİSİ DE NATIVE `<details>`/`<summary>` (checkbox/`<input>` YOK, JS gerekmez; mobilde her biri BAĞIMSIZ açılıp kapanır — biri açıkken diğerini kapatmaz; `open` sayesinde JS silinse bile masaüstünde ve mobilde İLK AÇILIŞTA içerik zaten görünür). `<summary>` metinleri KISA olsun, "Description" içeriğin ne kadar zengin olduğunu ima eden bir alt ibare taşısın.',
  isKeycapSet
    ? '4) `.bm-grid>details.bm-acc-desc>.bm-c2>.bm-inner> <div class="bm-info bm-reveal">` şu bölümleri SIRAYLA içerir: `<p class="bm-lede">` güçlü 3-4 cümle (anahtarlar `<strong>`) — İLK CÜMLE KISA olsun ("with this keycap set" yeter, "with this PBT dye-sublimated keycap set" gibi uzun tamlama YAZMA) ve "board" yerine "keyboard"/"mechanical keyboard" ile başla (board\'u sonraki cümlelerde kullanabilirsin); `<p class="bm-trivia">` ürünle ilgili 1 kısa ilginç bilgi (`<strong>` vurgulu); `<h3>Highlights</h3>`+`<ul class="bm-feat">` 5-6 `<li>` (`<span class="ico">EMOJI</span><span class="tx"><b>Başlık</b><span class="t">fayda</span></span>`); `<h3>Compatible Layouts</h3>`+`<div class="bm-layouts">` + `<div class="bm-layouts bm-layouts-keys">` (aşağıdaki İKİ SATIR kuralı) + `<p class="bm-layouts-note">` not; `<h3>Specifications</h3>`+`<div class="bm-spec">` (aşağıdaki SPECIFICATIONS kuralı — `<span class="bm-sub">…</span>` mantıklı kümelerle gruplanmış 12-16 `<div class="bm-r"><span class="bm-k">Etiket</span><span class="bm-v">Değer</span></div>`, GERÇEK veriler); `<h3>Why PBT Over ABS</h3>` (veya ürüne uygun bir "neden bu / X vs Y" başlığı)+`<table class="bm-compare">` 4 satırlık karşılaştırma (`<td class="bm-yes">` üstün tarafta); `<h3>Compatibility &amp; Care</h3>`+`<div class="bm-faq">` 5-7 tane NATIVE `<details class="bm-faq-item" name="bm-faq">` (HEPSİNDE `name="bm-faq"` — native tek-açık akordeon; ilkinde ayrıca `open`) → `<summary class="bm-faq-q"><span>SORU</span><span class="bm-plus"></span></summary><div class="bm-faq-a"><p>CEVAP</p></div></details>` (JS YOK — `<button>` + `is-open` DEĞİL; Shopify `<script>`\'i siler, JS akordeon AÇILMAZ); `<div class="bm-note"><b>📦 In the box:</b> ürünün GERÇEK kutu içeriği (örnek "keycap set, keycap puller, and a surprise-coupon thank-you card" cümlesi SADECE BİR ÖRNEK — ürünün gerçek veri/varyant/kaynak bilgisinde farklı bir şey varsa (ör. ekstra bir kapak, farklı bir hediye, aksesuar setinin kendine özgü parçaları) ONU yaz, körü körüne kopyalama).<br><b>💬 Compatibility questions?</b> please contact us.</div>` (KISA — "Before you order"/"Packaging" bloğu EKLEME; "message the store" YAZMA, "please contact us" kullan); `<div class="bm-cta"><p>kısa çağrı ✨</p><button type="button" data-bm-goto-atc onclick="…">🛒 Add to Cart</button></div>` — `onclick` içine ÖRNEK açıklamadaki CTA butonunun `onclick`\'ini AYNEN kopyala (Shop Pay / "Buy now" / "Buy with" butonlarını eleyip gerçek Add to Cart\'ı bulan uzun IIFE; ona kaydırıp arkasını 2 kez / 2.6s parlatır); `<div class="bm-trust">` 3 `<span>` güven rozeti (kutu + hover stilini örnekteki CSS verir).'
  ...
  isKeycapSet
    ? "COMPATIBLE LAYOUTS — İKİ SATIR + KLAVYE ÖRNEKLERİ: `<div class=\"bm-layouts\">` yüzde/boyut çipleri (`60%`,`65%`,`75%`,`TKL`,`96%`,`100%`,`Alice` — ürüne UYANLAR); hemen altında `<div class=\"bm-layouts bm-layouts-keys\">` tuş SAYISI çipleri; sonra `<div class=\"bm-compat-eg\">` içinde HER boyut için `<div class=\"bm-eg\"><b>60%</b><span>Model A · Model B · Model C</span></div>` — o boyutta GERÇEKTEN üretilen, yaygın bilinen 3 GERÇEK klavye modeli (örnekteki listeyi kullanabilirsin: 60% Anne Pro 2 / Ducky One 3 Mini / RK61 · 65% Keychron K6 / Ducky One 3 SF / RK68 · 75% GMMK Pro / Keychron K3 / NuPhy Air75 · TKL Ducky One 3 TKL / Leopold FC750R / Keychron C3 Pro · 96% Keychron Q5 / RK96 / Keychron K4 · 100% Keychron K10 / Ducky One 3 Full-Size / Leopold FC900R). UYDURMA model adı YASAK; 'reference only, not an endorsement' de."
    : "",
  isKeycapSet
    ? "COMPATIBILITY NOTU: `<p class=\"bm-layouts-note\">` içine ürünün TÜM tuşlarının göründüğü görsele (genelde 2. görsel) bakarak SADECE görselde GERÇEKTEN görünen spacebar boyutlarını yaz (3U görünmüyorsa 3U YAZMA), stepped Caps / split shift gibi özel tuşlar, uyumlu klavye tipi örnekleri (Alice, tri-mode 75%, standart TKL). Sonuna: emin değilse müşteri bizimle iletişime geçebilir. Görmediğin şeyi yazma."
    : "",
  isKeycapSet
    ? "ISO/ANSI DİLİ: 'Enter key' / 'Enter keycap' / tek başına 'Enter' kelimesi HİÇBİR YERDE (başlık, highlights, spec, FAQ, not) GEÇMEYECEK. Sadece 'ANSI & ISO layout compatible' / 'supports both ANSI and ISO layouts' gibi GENEL layout-uyumluluğu ifadesi kullan. Spec tablosunda ayrı bir 'Enter keys' satırı OLMAYACAK — 'Layout support: ANSI & ISO layout compatible' yeter. Bölgesel (UK/DE) ayrıntı ekleme."
    : "",
  isKeycapSet
    ? "PROFİL SCULPT — ARAŞTIRILMIŞ GERÇEK VERİ: SCULPTED (row-specific, her sırada FARKLI yükseklik/açı) profiller: Cherry, OEM, SA, MAO, FOA, SOA. UNIFORM (tüm sıralarda AYNI kap şekli/yüksekliği, sıradan sıraya değişmez) profiller: XDA, DSA, KAM, KOA, MOA, DMA, KCA, EAO. DİKKAT: MAO ile MOA'yı KARIŞTIRMA — isimleri neredeyse aynı ama MAO sculpted, MOA uniform'dur, iki AYRI profildir; operatörün yazdığı harflere dikkatle bak. Spec ve karşılaştırma tablosunda İKİ grup için de 'row-by-row sculpted' YAZMA — sculpted grubu 'row-specific', uniform grubu 'same shape on every row' diye ayır. Operatörün verdiği GERÇEK profil adına göre doğru gruba yaz."
    : "",
  "SPECIFICATIONS — KUTU ZORUNLU, DÜZGÜN, ŞIK: `<div class=\"bm-spec\">` KESİNLİKLE örnekteki gibi kenarlıklı/arka planlı TEK BİR KART olacak (`.bm-spec{border:1px solid var(--line);border-radius:12px;overflow:hidden;background:var(--milk)}`) — çıplak/kenarlıksız düz metin listesi YASAK. İçinde `<div class=\"bm-r\"><span class=\"bm-k\">Etiket</span><span class=\"bm-v\">Değer</span></div>` satırları, HER satır ince bir alt çizgiyle ayrılmış (`border-bottom:1px solid var(--line)`, son satırda yok) ve `:hover`'da hafif arka plan değişimi olacak (`.bm-spec .bm-r:hover{background:var(--sky)}`) — örnekteki `.bm-spec` CSS'ini AYNEN kullan, kendi kutusuz versiyonunu uydurma. Etiket ve değeri MUTLAKA ayrı `<span class=\"bm-k\">`/`<span class=\"bm-v\">` içine koy (yoksa 'MaterialPBT plastic' gibi bitişik görünür). Yalnız GERÇEK veri; bilinmeyen değer için satırı hiç yazma. GRUPLAMA (bm-sub başlıkları) OPSİYONEL — SADECE bu: veri doğal olarak 3-4 anlamlı kümeye ayrılıyorsa `<span class=\"bm-sub\">Küme adı</span>` ile grupla (12-16 satır); veri azsa DÜZ bir liste yap (7-10 satır, `bm-sub` YOK) — ama İKİSİNDE DE kart kenarlığı/arka planı/hover AYNI KALIR, sadece gruplama var/yok değişir. YASAK SATIRLAR: 'Brand'/'Marka' satırı YAZMA (satıcı/üretici markası açıklamada HİÇ geçmesin — başlıkta zaten varsa orada kalsın, spec tablosuna tekrar KOYMA); keycap seti/klavye aksesuarıysa 'Keyboard included' / 'Includes keyboard' gibi 'klavye dahil mi' satırı da YAZMA (gereksiz — zaten klavye değil, keycap/aksesuar sattığın açık).",
  isKeycapSet
    ? "KATEGORİ BİLİRKİŞİLİĞİ — highlights, FAQ ve karşılaştırma tablosundaki HER İDDİAYI keycap/mekanik klavye kategorisi bilgisiyle doğrula; jenerik dolgu değil, ürüne gerçekten alakalı ve doğru olsun. Örn: dye-sublimation PBT legend'ler ŞEFFAF/shine-through DEĞİLDİR (arkadan ışık glyph'in etrafından sızar, içinden değil) — 'shadow-light legends' / 'translucent legends' gibi UYDURMA terim KULLANMA. Profil (Cherry/OEM/SA/KOA…), legend işlemi (dye-sub/doubleshot), malzeme (PBT/ABS), stem (MX cross) terimlerini doğru kullan. Verilen KATEGORİ ARAŞTIRMASI varsa ona uy; kanıtlanamayan/şüpheli iddiayı YAZMA."
    : "KATEGORİ BİLİRKİŞİLİĞİ — highlights, FAQ ve karşılaştırma tablosundaki HER İDDİAYI ürünün GERÇEK kategorisi bilgisiyle doğrula (malzeme, kullanım amacı, bakım); jenerik dolgu değil, ürüne gerçekten alakalı ve doğru olsun. Keycap/klavye terimi (PBT/ABS/MX/profil/ANSI-ISO) bu ürün için KULLANMA. Verilen KATEGORİ ARAŞTIRMASI varsa ona uy; kanıtlanamayan/şüpheli iddiayı YAZMA.",
  isKeycapSet
    ? "\"board\" KELİMESİ: her paragrafta tekrarlanmasın. Çoğu yerde \"keyboard\" / \"mechanical keyboard\" kullan; \"board\" ara sıra geçebilir ama baskın olmasın."
    : "",
  "İLETİŞİM NOTU: FAQ'nun ilk cevabına VE/VEYA `bm-note`'a kısa bir satır ekle — uyumluluk sorusu olan müşteri bizimle iletişime geçebilir / sipariş öncesi yazabilir. İfade HER ZAMAN 'please contact us' / 'feel free to contact us' tarzı olsun — 'message the store' YAZMA.",
  isKeycapSet
    ? "FAQ — UYUMLULUK SORUSU KISA OLSUN: 'Will this fit my keyboard?' tarzı soruya UZUN teknik liste YAZMA (bottom row boyutu, spacebar ölçüsü, stepped Caps Lock gibi ayrıntıları FAQ CEVABINDA sıralama — bu ayrıntılar zaten `bm-layouts-note`'ta var, FAQ'ta TEKRAR ETME). FAQ cevabı SADECE: setin hangi genel boyutlara uyduğunu 1 kısa cümleyle söyle, sonra müşteriden klavyesinin bir LİNKİNİ ya da FOTOĞRAFINI göndermesini iste — 'gerisini biz hallederiz/kontrol ederiz' tarzı kısa bir kapanışla bitir. 2-3 cümleyi geçme."
    : "",
  isKeycapSet
    ? "FAQ — 'KLAVYE DAHİL Mİ?' SORUSU: cevap ŞÖYLE kurgulansın — hayır, bu listing SADECE keycap seti içindir; fotoğraflarda görünen klavye gövdesi/switch/başka objeler SADECE gösterim/demo amaçlıdır ve satışa DAHİL DEĞİLDİR. 'Your existing keyboard/switches/puller are not included' gibi kafa karıştırıcı, tersten kurulmuş bir cümle YAZMA — vurgu HER ZAMAN 'fotoğraftaki diğer objeler dahil değil, sadece keycap seti dahil' yönünde olsun."
    : "",
  isKeycapSet
    ? "FAQ — BACKLIT/RGB SORUSU KOŞULLU: 'Will my RGB shine through the legends?' tarzı bir soru SADECE ürünün kaynak verisinde/başlığında/açıklamasında backlit/RGB/shine-through bir iddia veya ilgili özellik GEÇİYORSA anlamlıdır — geçmiyorsa bu soruyu ya hiç YAZMA ya da nötr bir cümleyle geç (dye-sub PBT legend'lerin şeffaf olmadığını belirt, ürünün RGB/backlit ÖZELLİĞİ olduğunu VARSAYMA/UYDURMA). Sabit bir RGB cevabını her üründe ŞABLON gibi tekrarlama; kaynak veriye göre uyarla."
    : "",
  "ATC GLOW: `.bm-atc-glow` için KENDİ `@keyframes` / kural YAZMA — tek tanım sistemin sonda eklediği stil bloğunda. Glow SADECE `box-shadow` + hafif `scale()` içerir; ASLA `border-radius`/`padding`/`width`/`height`/`font-size`/`background` gibi butonun şeklini değiştiren özellik İÇERMEZ. Sistem: CTA'ya tıklayınca ürün sayfasındaki gerçek Add to Cart'a yumuşak kaydırır, kaydırma BİTİNCE glow'u baştan sonra 2 kez / 2.6s oynatır, sonra class'ı DOM'dan kaldırır (buton tıklama öncesiyle piksel-piksel aynı).",
  "LAYOUT — KESİN: `.bm-grid` masaüstünde 2 sütun (`minmax(0,1.35fr) minmax(0,1fr)`); `.bm-media` HER ZAMAN SOLDA (`grid-column:1`), `.bm-c2` (yazı/bilgi) HER ZAMAN SAĞDA (`grid-column:2`). Mobilde (≤899px) tek sütun ama sıra: ÖNCE görsel (`.bm-media` `grid-row:1`), SONRA yazı (`.bm-c2` `grid-row:2`). Bu sıralamayı hiçbir bölümde (Specifications, FAQ dahil) bozma. Sistem bunu `!important` ile de kilitliyor ama sen de doğru yaz.",
  "ÜLKE BAYRAĞI EMOJİSİ YASAK (🇳🇱 🇯🇵 🇺🇸 🏴 vb.) — anlamsız ve gereksiz; hiçbir yerde kullanma.",
  "ULUSLARARASI TON — KESİN: kaynak Çince başlık/açıklamadaki bir kelimeyi SESLETEREK Latin harflerine dökme (ör. 咕嘟嘟 → \"Gudongdong\"/\"Gududu\" gibi uydurma bir 'marka adı' üretme) YASAK — İngilizce okuyan biri için anlamsız ve yabancı durur. Çince kaynaktaki ses taklidi/deyim gibi unsurları YA doğrudan İngilizce anlamıyla çevir (ör. \"bubbling/simmering\" sesi) YA DA hiç bahsetme. Tema/koleksiyon adı, seri adı, ürün adı TAMAMEN hedef dilde ve uluslararası okunaklı olsun; Çince marka/mağaza adı da dahil hiçbir Çince kaynaklı özel isim ses-taklidi olarak taşınmasın.",
  ...
  "OKUNAKLI HTML: çıktıyı tek satıra sıkıştırma. Her etiket kendi satırında, iç içe elemanlar girintili; `<style>` içinde her kural yeni satırda; `<script>` yazmıyorsun zaten. Amaç: kopyalayınca insan gözüyle okunup debug edilebilsin.",
  "ESNEKLİK: Yukarıdaki KELİME/ÜSLUP tercihleri (ISO ifadesi, ilk cümle uzunluğu, 'board' yerine 'keyboard', spec küme adları, 'with this keycap set') KATI KURAL DEĞİL — ruhuna uy, daha iyisini bulursan onu yaz, bunlar yüzünden çıktıyı bloklama. KATI olanlar: native `<details>` FAQ, kendi `<script>` yok, `.bm-reveal` görünür, ülke bayrağı yok, `data-bm-goto-atc` + doğru `onclick`, `.bm*` sınıf adları, bölüm seti, uzunluk bandı.",
  ]
    .filter(Boolean)
    .join(" ");
}
...
/**
 * Manus's `/v2/task.create` rejects a kickoff message over ~5000 estimated
 * tokens — nothing to do with how long the HTML Manus eventually WRITES back
 * (that comes later, polled over the task's run, and can be as long as it
 * needs). The full shopifyDescRuleStacked()/shopifyDescRuleOther() rules plus
 * a 40,000-char reference example were blowing past that on every call, so
 * Manus always failed and the code silently kept the first-pass Claude
 * description — this is the compact stand-in used ONLY for that kickoff
 * message, condensing the same hard structural constraints without the
 * prose or the full example.
 */
function manusDescRuleCompact(isStacked: boolean, isKeycapSet: boolean): string {
  if (isStacked) {
    return [
      ...
      "Kök `<div class=\"bm\">` İÇİNDE SIRAYLA: (a) bm-hero HER ZAMAN DOĞRUDAN GÖRÜNÜR (accordion İÇİNDE DEĞİL) — bm-eyebrow + h2 + sub + bm-badges 4-6 KISA rozet (masaüstünde `flex-wrap:nowrap` + `overflow-x:auto` ile tek satır, mobilde `@media max-width:899px{.bm-badges{flex-wrap:wrap!important}}` ile sarmasına izin ver — rozet metni asla yarım/kırpık görünmesin); (b) `<div class=\"bm-grid\">` İÇİNDE TAM OLARAK İKİ NATIVE `<details class=\"bm-acc bm-acc-desc\" open><summary class=\"bm-bar\">📝 Description — full specs, compatibility &amp; FAQ inside<i></i></summary>…</details>` VE `<details class=\"bm-acc bm-acc-media\" open><summary class=\"bm-bar\">🖼️ Images<i></i></summary><div class=\"bm-media\"></div></details>` (checkbox YOK, `open` ZORUNLU, ikisi BAĞIMSIZ açılır/kapanır). Description details İÇİNDE SIRAYLA: bm-lede + bm-trivia · Highlights (`<ul class=\"bm-feat\">`, her `<li>`: `<span class=\"ico\">emoji</span><span class=\"tx\"><b>başlık</b><span class=\"t\">fayda</span></span>`, 5-6 tane)."
        + (isKeycapSet
          ? " Compatible Layouts (`bm-layouts` boyut çipleri + `bm-layouts bm-layouts-keys` tuş sayısı çipleri + `bm-compat-eg` içinde her boyut için 3 GERÇEK klavye modeli, uydurma isim yok)."
          : "")
        + " Specifications (`bm-spec`, her satır `bm-r>bm-k+bm-v`, 7-16 satır sadece gerçek veri — veri 3-4 kümeye doğal ayrılıyorsa `bm-sub` alt başlıklarıyla gruplu, azsa düz liste) · karşılaştırma tablosu (`bm-compare`, üstün tarafta `bm-yes`) · Compatibility & Care: 5-7 NATIVE `<details class=\"bm-faq-item\" name=\"bm-faq\">` (hepsinde `name=\"bm-faq\"`, ilkinde ayrıca `open`, JS/button YOK) · `bm-note` (kısa: kutu içeriği + iletişim notu) · `bm-cta` (`data-bm-goto-atc` attribute'u + kısa çağrı metni, buton `onclick` YAZMA — sistem ekler) · `bm-trust`.",
      "`.bm-media` içine KENDİN `<img>` yazma, boş bırak — sistem ekler. Kendi `<script>` YAZMA.",
      "KATI: `.bm-reveal` başta görünür olmalı (opacity:1) — JS'e bağımlı gizleme yok. 'Enter key'/'Enter'/tek başına Enter YASAK, 'ANSI & ISO layout compatible' kullan. Ülke bayrağı emoji YASAK. PROFİL SCULPT (araştırılmış gerçek veri): Cherry/OEM/SA/MAO/FOA/SOA = sculpted (row-specific); XDA/DSA/KAM/KOA/MOA/DMA/KCA/EAO = uniform (aynı şekil) — MAO'yu MOA ile KARIŞTIRMA, ikisi farklı (MAO sculpted, MOA uniform). `.bm{}` DIŞINDA sabit hex/rgb yazma, hepsi `var(--…)`; RENK — KESİN: palet EKLİ GERÇEK ÜRÜN FOTOĞRAFLARINA göre seçilir, tema metninden tahmin edilmez (turuncu değilse turuncu kullanma) — istisnası yok. Spec tablosunda Brand/Marka satırı YOK; keycap/aksesuarsa 'Keyboard included' satırı YOK. `bm-note`'ta iletişim ifadesi 'message the store' DEĞİL 'please contact us' olsun; kutu içeriği ürünün GERÇEK içeriğine göre yaz, örnek metni ezbere kopyalama.",
      "Emoji taşıyan hover'larda `transform:rotate(15deg)` + yumuşak `transition` (cubic-bezier), ani geçiş yok. Rozet (`bm-badges span`) hover'ında `transform`/`box-shadow` YAZMA, sadece background/border-color (satır yatay kayar, taşan gölge kırpılır).",
      "Tüm çıktı belirtilen hedef dilde, Çince kalmasın. Kaynak Çince kelimeyi sesleterek uydurma Latin-harfli isim üretme (\"Gudongdong\" gibi) — ya gerçek anlamıyla çevir ya hiç yazma. Markdown/kod bloğu yok.",
    ]
      .filter(Boolean)
      .join(" ");
  }
  return [
    ...
    "Bölümler SIRAYLA: rozet şeridi · HERO (başlık+slogan+emoji) · INTRO (2-3 paragraf) + görsel ızgarası · FEATURES (4-8 madde, ikonlu) · SPOTLIGHT (tek geniş görsel) · varsa VARYANT kartları"
      ...
      + " · SPECS tablosu (gerçek veri, etiket/değer ayrı hücrede) · UYUMLULUK notu · KUTU İÇERİĞİ (kısa) · FAQ · CTA (en son).",
    `Her tekli görsel için \`<div class="pd-media"></div>\` slotu aç (KENDİN \`<img>\` yazma, sistem doldurur) — TEK BAŞINA/izole kare slot YASAK, ya ızgarada birden fazla ya da yanında metinle.`,
    "FAQ = NATIVE `<details class=\"pd-faq-item\" name=\"pd-faq\">` (hepsinde `name=\"pd-faq\"`, ilkinde `open`), JS/button YOK, 5-7 soru.",
    "CTA: `<button type=\"button\" class=\"...\" data-pd-goto-atc>METİN</button>` (en son bölüm, `onclick` YAZMA — sistem ekler).",
    "KATI: kendi `<script>` yok, `*-reveal` varsa başta görünür (opacity:1), ülke bayrağı emoji yok, 'Enter key'/'Enter' yasak → 'ANSI & ISO layout compatible', Cherry=row-specific/diğer profil=uniform ayrımı doğru, `.pd-<önek>{}` dışında sabit hex yok (hepsi var(--pd-…)), bölüm başlıkları hedef dilde (Çince yok), Brand/Marka satırı YOK, keycap/aksesuarsa 'Keyboard included' satırı YOK.",
    "Emoji hover'da `rotate(15deg)` + yumuşak transition.",
  ]
    .filter(Boolean)
    .join(" ");
}
...
export async function generateListing(
  ...
  // instead of just handing over the URL). The operator called color-matching
  // a KESİN (non-negotiable) rule and asked to remove limitations around it —
  // raised from a 2-image cap (originally "just a colour cue") to 4, so the
  // model sees more of the product's real palette across angles/variants
  // before picking one. One failed download is skipped rather than failing
  // the whole generation.
  const galleryImages = product.images.filter((im) => im.role === "gallery").slice(0, 4);
  ...
  const channelRules = isShopify
    ...
    : [
        ...
        "  3) 2. ANAHTAR KELİME: 1. ürün adının bir varyantı (1. \"keycap set\" ise 2. \"keycaps\"), EN AZ 2 KELİMEDEN oluşan gerçek bir ürün ifadesi olmalı — TEK BAŞINA çıplak bir kategori kelimesi (\"Backpack,\", \"Case,\", \"Bag,\" gibi) ASLA olamaz. Bu kelime ürünü DOĞRU tanımlamalı; ürün gerçekten öyle değilse (ör. bir çanta \"rucksack\" değilse) o kelimeyi kullanma. Sonuna virgül koy.",
        "  4) Ardından ürünü tanıtan diğer önemli öbekler, aralarında VİRGÜL ile (örn. \"Cherry Profile, PBT Dye-Sub, ...\") — HER öbek bu SPESİFİK ürün için doğru ve gerçek olmalı, sadece doldurmak için yakın/alakasız eş anlamlı kelime EKLEME.",
  ...
  if (wantManusDesc || wantClaudeDesc) {
    try {
      ...
      if (wantManusDesc) {
        // Manus's task-kickoff message has its own hard ~5000-estimated-token cap
        // (separate from, and much smaller than, however long Manus's eventual
        // HTML reply is allowed to be) — a compact rule + trimmed context instead
        // of the full Claude prompt + 40,000-char example, which always blew
        // past it and made every Manus attempt fail silently.
        const mSys = [
          "Sen bir Shopify HTML açıklama uzmanısın. SADECE `description` alanını üret — tam, stillendirilmiş HTML. Başlık/etiket ÜRETME.",
        ...
        const mUsr = [
          `ÜRÜN: ${dTitle}`,
          `TÜR: ${input.productType || "?"}`,
          input.globalRules?.trim() ? `KURAL: ${input.globalRules.trim().slice(0, 300)}` : "",
          input.productNote?.trim() ? `OPERATÖR NOTU (son söz): ${input.productNote.trim().slice(0, 500)}` : "",
          input.categoryResearch?.trim() ? `KATEGORİ: ${input.categoryResearch.trim().slice(0, 600)}` : "",
          `Özellikler: ${JSON.stringify(cleanPropsRecord(sv.product.props)).slice(0, 500)}`,
          `Varyantlar: ${JSON.stringify(product.variants.map((v) => v.nameTranslated || v.name)).slice(0, 300)}`,
          `Kaynak metin: ${product.descHtml.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").slice(0, 400)}`,
        ]
          .filter(Boolean)
          .join("\n");
        const mres = await runManusTask([{ type: "text", text: `${mSys}\n\n${mUsr}` }], {
          locale: "en",
      ...
      console.error("[generateListing] 2nd-pass description failed, keeping first-pass:", e);
    }
  }
