...
function manusDescRuleCompact(isStacked: boolean, isKeycapSet: boolean): string {
  if (isStacked) {
    return [
      ...
      "KATI: `.bm-reveal` başta görünür olmalı (opacity:1) — JS'e bağımlı gizleme yok. 'Enter key'/'Enter'/tek başına Enter YASAK, 'ANSI & ISO layout compatible' kullan. Ülke bayrağı emoji YASAK. PROFİL SCULPT (araştırılmış gerçek veri): Cherry/OEM/SA/MAO/FOA/SOA = sculpted (row-specific); XDA/DSA/KAM/KOA/MOA/DMA/KCA/EAO = uniform (aynı şekil) — MAO'yu MOA ile KARIŞTIRMA, ikisi farklı (MAO sculpted, MOA uniform). `.bm{}` DIŞINDA sabit hex/rgb yazma, hepsi `var(--…)`; ürün rengine uygun palet seç (turuncu değilse turuncu kullanma). Spec tablosunda Brand/Marka satırı YOK; keycap/aksesuarsa 'Keyboard included' satırı YOK. `bm-note`'ta iletişim ifadesi 'message the store' DEĞİL 'please contact us' olsun; kutu içeriği ürünün GERÇEK içeriğine göre yaz, örnek metni ezbere kopyalama.",
