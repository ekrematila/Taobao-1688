...
function shopifyDescRuleOther(descImgN: number, isKeycapSet: boolean): string {
  ...
  return [
    ...
    "ISO/ANSI DİLİ: 'ISO Enter' diye ayrı parça yazma — 'ANSI & ISO Layout Compatible' de. ISO Enter L şeklindedir (iki sıra), ANSI geniş tek-sıra bardır. ISO Enter'ı ASLA 'tall'/'long'/'uzun' diye tarif etme (arka plan bilgisi olarak bile sızmasın). Bölgesel ayrıntıları açıklamaya ekleme.",
...
const SHOPIFY_DESC_RULE_STACKED = [
  ...
  "ISO/ANSI DİLİ: ASLA 'ISO Enter' diye ayrı bir ürün/parça yazma. 'ANSI & ISO Layout Compatible' / 'ANSI & ISO layout' de. Enter tuşu hem ANSI hem ISO'da vardır; ISO'da L ŞEKLİNDEDİR (iki sıra kaplar), ANSI'de geniş tek-sıra bardır. ISO Enter'ı ASLA 'tall'/'long'/'uzun' diye tarif etme — sadece 'L-shaped'. 'tall'/'long' kelimeleri ISO Enter'ın YAKININDA bile geçmesin (arka plan bilgisi olarak bile sızmasın). Bölgesel (UK/DE) ayrıntıları açıklamaya ekleme. 2. görselde büyük L şeklinde Enter varsa ürün ISO uyumludur.",
  ...
  "ATC GLOW: `.bm-atc-glow` için KENDİ `@keyframes`'ini YAZMA — tek tanım sistemin sonda eklediği stil bloğunda (2 kez nabız, toplam 2.6s). CTA'ya tıklayınca sistem ürün sayfasındaki gerçek Add to Cart'a YUMUŞAK kaydırır, kaydırma TAM BİTİNCE glow'u baştan sonra oynatır (ortasından değil).",
    ...
    if (cur.length > 400 && /<style/i.test(cur)) {
      try {
        ...
        const rSys = [
          ...
          "4) CTA `<button data-bm-goto-atc>` (veya `data-pd-goto-atc`) inline `onclick` taşımalı; `onclick` mantığı Shop Pay / '.shopify-payment-button' / 'Buy now' / 'Buy with' butonlarını ELEMELİ ve GERÇEK Add to Cart'ı bulmalı (bkz. örnek). Yoksa/yanlışsa örnektekiyle DEĞİŞTİR. `.bm-atc-glow`/`.pd-atc-glow` için blokta `@keyframes` TANIMI OLMAMALI (çakışma) — tek tanımı sistem sonda ekliyor; varsa SİL. Statik `box-shadow !important` ile ezme.",
          ...
          "7) TERCİHEN: 'ISO Enter' yerine 'ANSI & ISO Layout Compatible'; ISO Enter'a 'tall'/'long'/'uzun' deme (L-shaped yeter). Zaten uygunsa dokunma.",
          "8) TERCİHEN: ilk cümle kısa; 'board' her paragrafta tekrar etmesin (çoğu 'keyboard'). Zaten iyiyse dokunma.",
          ...
          "NOT: 7 ve 8 TERCİH — yalnızca gerçekten gerekiyorsa dokun. Diğerleri (özellikle 2,3,4,6,9,15) KATI.",
