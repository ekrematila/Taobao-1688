...
function shopifyDescRuleOther(descImgN: number, isKeycapSet: boolean): string {
  ...
  return [
    ...
    'ZORUNLU — CTA: `<button type="button" class="<kendi-sınıfın>" data-pd-goto-atc onclick="var a=document.querySelector(\'form[action*=cart] button[type=submit],form[action*=cart] [type=submit],button[name=add],.product-form__submit,[data-add-to-cart],#AddToCart,.btn--add-to-cart\')||document.querySelector(\'.shopify-payment-button__button\');if(a){a.scrollIntoView({behavior:\'smooth\',block:\'center\'});setTimeout(function(){a.classList.remove(\'pd-atc-glow\');void a.offsetWidth;a.classList.add(\'pd-atc-glow\');setTimeout(function(){a.classList.remove(\'pd-atc-glow\');},2600);},650);}return false;">METİN</button>` — bu `onclick`\'i AYNEN kopyala (href="#" YAZMA, `<a>` KULLANMA). Tıklayınca ürün sayfasındaki GERÇEK Add to Cart butonuna yumuşak kaydırır, kaydırma bitince ONUN arkasını 2 kez / toplam 2.6s parlatır. `.pd-atc-glow` için KENDİ `@keyframes` YAZMA — tek tanımı sistem sonda ekler. CTA bloğu açıklamanın EN SON içerik parçası olsun.',
    ...
    if (cur.length > 400 && /<style/i.test(cur)) {
      try {
        ...
        const rSys = [
          ...
          "4) CTA `<button data-bm-goto-atc>` (veya `data-pd-goto-atc`) MUTLAKA şu inline `onclick`'i taşımalı: `var a=document.querySelector('form[action*=cart] button[type=submit],form[action*=cart] [type=submit],button[name=add],.product-form__submit,[data-add-to-cart],#AddToCart,.btn--add-to-cart')||document.querySelector('.shopify-payment-button__button');if(a){a.scrollIntoView({behavior:'smooth',block:'center'});setTimeout(function(){a.classList.remove('bm-atc-glow');void a.offsetWidth;a.classList.add('bm-atc-glow');setTimeout(function(){a.classList.remove('bm-atc-glow');},2600);},650);}return false;` (pd- düzeninde `pd-atc-glow`). Yoksa EKLE. `.bm-atc-glow`/`.pd-atc-glow` için `@keyframes` TANIMI blokta OLMAMALI (birden fazla tanım = çakışma) — tek tanımı sistem sonda ekliyor; varsa SİL. Statik `box-shadow !important` ile ezme.",
          ...
          "7) 'ISO Enter' diye ayrı parça YAZMA — 'ANSI & ISO Layout Compatible' yap. ISO Enter L şeklindedir (iki sıra); ASLA 'tall'/'long'/'uzun' deme — bu kelimeler ISO Enter'ın yakınında bile geçmesin. Bölgesel (UK/DE) ayrıntı ekleme.",
          "8) İLK CÜMLE kısa olmalı ('with this keycap set', 'with this PBT dye-sublimated keycap set' DEĞİL) ve 'board' yerine 'keyboard'/'mechanical keyboard' ile başlamalı. Gövde boyunca 'board' her paragrafta tekrarlanmamalı — çoğunu 'keyboard' yap.",
