...
function shopifyDescRuleStacked(isKeycapSet: boolean): string {
  ...
  '3) `<div class="bm">` sarmalayıcı, İÇİNDE `<details class="bm-acc" open><summary class="bm-bar">EMOJİ Product Details — full specs, compatibility &amp; FAQ inside<i></i></summary> ... </details>` — NATIVE `<details>`/`<summary>` (checkbox/`<input>` YOK, JS gerekmez, mobilde dokunma açar/kapatır, `open` sayesinde JS silinse bile masaüstünde ve mobilde İLK AÇILIŞTA içerik zaten görünür). `<summary>` metni "Product Details" ile birlikte KISA bir alt ibare içersin (ör. "— full specs, compatibility & FAQ inside") — kullanıcı içeride çok detay olduğunu tıklamadan anlasın. `<details>` İÇİNDE SIRAYLA `.bm-c1>.bm-inner> <div class="bm-hero bm-reveal">` (`.bm-eyebrow` seri/koleksiyon adı · `<h2>` başında+sonunda tema emojisi · `.sub` tek satır özet · `.bm-badges` 4-6 KISA (2-3 kelime) emoji\'li `<span>` rozet — MASAÜSTÜNDE TEK SATIRA sığmalı: `.bm-badges{display:flex;flex-wrap:nowrap;overflow-x:auto;overflow-y:hidden;scrollbar-width:none;-ms-overflow-style:none}` + `.bm-badges::-webkit-scrollbar{display:none}` yaz; ama MOBİLDE (`@media max-width:899px`) bunu `.bm-badges{flex-wrap:wrap!important;overflow:visible!important}` ile EZ — dar ekranda rozet metninin YARIM/KIRPIK görünmesi ("140 Key", "PBT Dye-S" gibi) KESİNLİKLE YASAK, tam metin ya tek satırda ya da 2-3 satıra sararak HER ZAMAN eksiksiz okunsun). Rozet `:hover`\'ında SADECE `background`/`border-color` değişsin — `transform`/`box-shadow` YAZMA (satır `overflow-x:auto` olduğu için taşan bir gölge/kaldırma efekti kutunun kenarında ÇİRKİN KIRPILIR). `.bm-hero::before/::after` dekoratif emoji glifi mobilde (`@media max-width:899px`) KÜÇÜLTÜLMELİ (`font-size` yaklaşık yarıya inmeli, `opacity` düşük kalmalı) — büyük haliyle dar ekranda rozet satırının ÜSTÜNE BİNEBİLİR, bunu asla bırakma.',
  isKeycapSet
...
function manusDescRuleCompact(isStacked: boolean, isKeycapSet: boolean): string {
  if (isStacked) {
    return [
      ...
      "Kök `<div class=\"bm\">` İÇİNDE `<details class=\"bm-acc\" open><summary class=\"bm-bar\">EMOJİ Product Details — full specs, compatibility &amp; FAQ inside<i></i></summary>...</details>` (NATIVE details/summary, checkbox YOK, `open` ZORUNLU) — bunun İÇİNDE SIRAYLA: bm-hero (bm-eyebrow + h2 + sub + bm-badges 4-6 KISA rozet — masaüstünde `flex-wrap:nowrap` + `overflow-x:auto` ile tek satır, ama mobilde `@media max-width:899px{.bm-badges{flex-wrap:wrap!important}}` ile sarmasına izin ver — rozet metni asla yarım/kırpık görünmesin) · bm-lede + bm-trivia · Highlights (`<ul class=\"bm-feat\">`, her `<li>`: `<span class=\"ico\">emoji</span><span class=\"tx\"><b>başlık</b><span class=\"t\">fayda</span></span>`, 5-6 tane)."
        + (isKeycapSet
